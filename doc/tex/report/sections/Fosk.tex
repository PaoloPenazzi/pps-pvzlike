...
\begin{enumerate}
    ...
    Quando uno \texttt{Zombie} è colpito da un \texttt{CherryBullet} potrebbe essere rallentato, per questo motivo si è deciso di
    mettere la velocità in ciascun costruttore per ciascun tipo di \texttt{Zombie} implementato.
    In questo modo, seguendo un approccio funzionale e favorendo l'immutabilità delle istanze, quando il \texttt{CherryBullet} collide con uno specifico \texttt{Zombie}
    ...
    il tipo \texttt{Bullet} sparato, la velocità di default, la velocità degli \texttt{Zombie} rallentati e un metodo per generare
    ...
    Nell'object \texttt{ZombieDefaultValues} sono definite le principali caratteristiche di ciascun tipo di \texttt{Zombie}: lo stato di default \texttt{Moving}, la vita iniziale, il tipo di \texttt{Bullet} a disposizione, le velocità di default e le velocità quando vengono rallentati.
    Il comportamento degli \texttt{Zombie} è incapsulato nel \texttt{TroopActor}.
    Il model degli \texttt{Zombie} è stato testato nella classe \texttt{ZombieModelTest}
